...
module.exports = (ctx) ->
  ...
  rQ = [
    [ 'where', 'meta.deleted', '==', false ]
    [ 'where', 'meta.type',    '==', 'registration/team-manager'  ]
    [ 'where', 'rel.team',     '==', ic.rel.team  ]
  ...
  [ league, manager_count, registration, season, ss, team, u2l, u2s, u2t, ] = await (all [
  ...
  if (any [
    (!team)
    (ss && ((get ss, 'val.registration_status.manager_team') == 'closed'))
    (manager_count >= ((get ss, 'val.team_limits.team_managers') ? 0))
  ...
  required    = ss.val.required_info.manager
  ...
  registration = (Registration.create {
    meta:
      id: registration_id
      type: 'registration/team-manager'
    rel:
  ...
  u2l = (U2L.create (merge (if u2l then u2l else {}), {
    ...
    rel:
      ...
      user:   user.obj().meta.id
    val: 
      roles: (uniq (union (get u2l, 'val.roles'), [ 'manager' ]))
  }))
  ...
  u2s = (U2S.create (merge (if u2s then u2s else {}), {
    ...
    rel:
      ...
      user:   user.obj().meta.id
    val: 
      roles: (uniq (union (get u2s, 'val.roles'), [ 'manager' ]))
  }))
  ...
  u2t = (U2T.create (merge {}, (ite u2t, u2t, {}), {
    ...
    val: 
      ...
      roles:        (uniq (union (get u2t, 'val.roles'), [ 'manager' ]))
  }))

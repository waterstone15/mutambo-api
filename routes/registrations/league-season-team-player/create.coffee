...
module.exports = (ctx) ->
  ...
  { form, illstp, code } = ctx.request.body

  if !uid
    ctx.unauthorized()
    return

  { fba, illstp, user, } = await hash({
    fba:   fbaI()
    illstp: fbaH.get('/invite-links', illstp.meta.id)
    user:  User.getByUid(uid, { values: { meta: ['id'] }})
  })
  { league, season, team } = await hash({
    league: fbaH.get('/leagues', illstp.rel.league)
    season: fbaH.get('/seasons', illstp.rel.season)
    team:   fbaH.get('/teams', illstp.rel.team)
  })
  ...
  registration =
    meta:
      ...
      type: 'registration-league-season-team-player'
      created_at: now
      updated_at: now
      deleted: false
      v: 4
